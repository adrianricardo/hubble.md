...
is an unauthenticated `ConvexHttpClient`**. `listWorkspaces`/`getLiveDocuments` call
`requireWorkspaceMember`, which needs an authenticated user. So "real sign-in"
requires: (1) a Convex client + Convex Auth in the renderer, and (2) plumbing the
auth **token** through `connectSyncedFolder` into the main process so
`createConvexBackend` can `setAuth(token)`. That contract spans renderer + main +
`convex-client` and is the premier gate (RT1).
...
|----|------|------|-----------|-------|
| RT1 | Desktop Convex client + Convex Auth + main-process backend auth token plumbing | **premier** | — | `tasks/RT1-desktop-auth-and-backend-token.md` |
| RT2 | Settings "Synced Folder" section: workspace context, folder pick, connect/disconnect, status, event toasts | **standard** | RT1 | `tasks/RT2-settings-synced-folder-section.md` |
| RT3 | Sync-root grant + first-run-on-existing-folder safety guard (§6 case 5) | **standard** | RT2 | `tasks/RT3-root-grant-and-first-run-guard.md` |
| RT4 | User-facing copy pass: toast/status strings for every synced-folder event | **economy** | RT2 | `tasks/RT4-event-copy-pass.md` |
| RT5 | Human test runbook + Convex seed/smoke script | **standard** | — (verify after RT2) | `tasks/RT5-test-runbook-and-smoke.md` |
...
RT1 (premier, gate)
 └─► RT2 (standard)
      ├─► RT3 (standard)   ┐ parallel (disjoint: RT3 = grant/first-run logic,
      └─► RT4 (economy)    ┘            RT4 = strings/toasts)
RT5 (doc + script) — author in parallel from the start; run/verify after RT2.
```

RT1 is the only true gate. RT3 and RT4 are disjoint and parallelizable once RT2
lands. RT5's runbook can be written immediately; its smoke script is verified once
RT2 makes the flow reachable.
