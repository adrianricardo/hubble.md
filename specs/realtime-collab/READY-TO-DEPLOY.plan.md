...
| RD2 | `Shared with me/` materialization | **standard** | RT1 | Thread `documents.listSharedWithMe` → `convex-client` `getSharedWithMe` → `materializeSyncedFolder`; backend + `by_user` index already exist. |
...
| RD9 | Packaged desktop release | **standard** | RD1,RD4 | electron-vite production build, sign/notarize, auto-update channel (`UpdatesSection` already exists), install smoke. |
| RD10 | Rebase/merge `spike/prosemirror-sync` → fork `main` behind a feature flag | **premier** | RD3–RD8 | ~30 commits; flag-gate the Live-Document/synced-folder surfaces; CI green; keep legacy file-authoritative paths untouched (ADR-0009). |
...
| RD12 | MCP server for the patch API (Stage 4 deferred) | **economy** | RD3 | Optional for deploy; the `hubble cloud document get/patch/reconcile` CLI already covers headless agents. Cut if not needed. |
